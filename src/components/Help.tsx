...
import { FaQuestion, FaEnvelope, FaBook, FaComments, FaTimes, FaSearch } from 'react-icons/fa';
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="mb-8 flex justify-center">
          <div className="relative w-full max-w-xl">
            <input
              ...
              className="w-full px-4 py-2 rounded-full border-2 border-blue-500 focus:outline-none focus:border-blue-600 dark:bg-gray-700 dark:text-white"
            />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {filteredSections.map((section, index) => (
            <motion.div
              key={index}
              className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <p className="text-gray-600 dark:text-gray-300 mb-4 text-center">{section.description}</p>
              <button 
                onClick={() => handleSectionClick(index)}
                className="w-full bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 transition-colors duration-300 transform hover:scale-105"
              >
                Learn More
              </button>
            </motion.div>
